...
const username = document.getElementById('username');      // name of message sender
const content = document.getElementById('content');        // message input field
const send = document.getElementById('send');      // send button

const channel = socket.channel('chat_room:lobby', {});  // connect to chat "room"
...
function sendMessage() {
  ...
  channel.push('new_client_message', {        
    username: username.value || "guest", // get value of "name" of person sending the message. Set guest as default
    content: content.value,          // get message text (value) from msg input field.
    inserted_at: new Date()      // date + time of when the message was sent
  });

  content.value = '';                // reset the message input field for next message.
...
send.addEventListener('click', function (event) {
  if (content.value.length > 0) { // don't sent empty msg.
    sendMessage()
  }
});
